...
<script language='javascript'>
		function consultar(fecha1, fecha2) {
            document.frm_listPesos.mifecha1.value = fecha1;
            document.frm_listPesos.mifecha2.value = fecha2;
			alert(fecha1, fecha2);
            document.frm_listPesos.submit();
		}
        </script>
    ...
    <div id="contenedor">
      ...
      <div id="centrado">
          <!--/*********************************FORMULARIO PARA EL FILTRO*****************************************************/ -->
                             <form method="post" action="reportesPorDia.php" name="form_filtro" id="form_filtro" style="align-items: center; background:rgba(0,0,0,0.0);">
...
?>
        ...
        <section style="text-align: center; margin: 0 auto; height: 60%">
            <article style="width:85%; height: 100%;text-align: center; margin: 0 auto;">
                 ...
                 <form method="post" action="reportesPorDia.php" name="frm_fechas" id="frm_fechas" style="width: auto; height: auto;">
                        <h4> Elija un Rango de fechas<input type="date" id="fecha1" name="fecha1"> <input type="date" id="fecha2" name="fecha2"></input> <input type="submit" value="Buscar"></h4>
                </form>

                 <form method="post" action="clases/registro.php" name="frm_listPesos" id="frm_listPesos" style="width: auto; height: auto;">
					            <!-- <input type="hidden" id="micodigo" name="micodigo"> -->
                      <?php
                        echo"
                        <h4> Elija un Rango de fechas<input type='text' id='fecha1' name='fecha1' value='$fecha1'> <input type='text' id='fecha2' name='fecha2'  value='$fecha2'></input> <input type='submit' value='Buscar'></h4>
                                   ";
                                    include_once 'clases/registro.php';
                                    $regist = new Registro();
                                    $registros = $regist->listarInformePorRangoFecha($fecha1, $fecha2);
                                   if($registros){
                                    echo "
